...
$MYSQL -u $MUSER -h $MHOST -e "use $MDB"  &>/dev/null
if [ $? -ne 0 ]
then
...
TABLES=$($MYSQL -u $MUSER -h $MHOST $MDB -e 'show tables' | $AWK '{ print $1}' | $GREP -v '^Tables' )
...
do
	...
	$MYSQL -u $MUSER -h $MHOST $MDB -e "drop table $t"
done